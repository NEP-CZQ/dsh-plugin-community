  ...
  factory: function (require) {
    ...
    function CommunitySection() {
      ...
      function onUpdate(id) {
        setUpdateBusy(function (b) { return Object.assign({}, b, { [id]: 'updating' }); });
        post('/plugin-community/update', id).then(function (res) {
          if (res && res.executed) {
            setUpdateBusy(function (b) { return Object.assign({}, b, { [id]: { done: true, command: res.command } }); });
          } else if (res && res.command) {
            setUpdateBusy(function (b) { return Object.assign({}, b, { [id]: { command: res.command, stagedPath: res.stagedPath, stderr: res.stderr || res.error || null } }); });
          } else {
            var m4 = (res && (res.error || res.stderr)) || '更新失败';
            setUpdateBusy(function (b) { return Object.assign({}, b, { [id]: { error: m4 } }); });
          }
        }).catch(function (err) {
      ...
      function updatesTab() {
        ...
        return React.createElement('div', { className: 'pcs-root' },
          data.updates.map(function (u) {
            ...
            var extraHint = null;
            if (ub === 'updating') {
              right = React.createElement('button', { className: 'pcs-btn', disabled: true }, '执行中…');
            } else if (ub && ub.done) {
              right = React.createElement('button', { className: 'pcs-btn done', disabled: true }, '已更新');
            } else if (ub && ub.command) {
              right = React.createElement('span', { className: 'pcs-hint' }, '待授权执行');
              extraHint = React.createElement('div', { className: 'pcs-hint' }, '命令：' + ub.command + (ub.stderr ? ' · ' + ub.stderr : ''));
            } else if (ub && ub.error) {
              right = React.createElement('button', { className: 'pcs-btn', onClick: function () { onUpdate(u.id); } }, '重试');
              extraHint = React.createElement('div', { className: 'pcs-error' }, ub.error);
            } else {
              right = React.createElement('button', { className: 'pcs-btn', onClick: function () { onUpdate(u.id); } }, '更新');
            }
      ...
      var communityTab = React.createElement('div', { className: 'pcs-root' },
        React.createElement('div', { className: 'pcs-sub' }, '精选（官方）：一键安装。社区：实时同步 GitHub topic「dsh-plugin」。已装插件不再显示。'),
