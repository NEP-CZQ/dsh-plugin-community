    ...
    function CommunitySection() {
      ...
      function onUpdate(id) {
        ...
        post('/plugin-community/update', id).then(function (res) {
          if (res && res.ok) {
            setUpdateBusy(function (b) { return Object.assign({}, b, { [id]: { command: res.command, stagedPath: res.stagedPath } }); });
          } else {
            var m4 = (res && res.error) || '更新失败';
      ...
      function updatesTab() {
        ...
        return React.createElement('div', { className: 'pcs-root' },
          data.updates.map(function (u) {
            ...
            var extraHint = null;
            if (ub === 'updating') {
              right = React.createElement('button', { className: 'pcs-btn', disabled: true }, '生成中…');
            } else if (ub && ub.command) {
              right = React.createElement('span', { className: 'pcs-hint' }, '待授权执行');
              extraHint = React.createElement('div', { className: 'pcs-hint' }, '命令：' + ub.command);
            } else if (ub && ub.error) {
