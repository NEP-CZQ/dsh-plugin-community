      ...
      function installedTab() {
        ...
        return React.createElement('div', { className: 'pcs-root' },
          data.installed.map(function (it) {
            var srcLabel = it.source === 'official' ? '官方' : (it.source === 'community' ? '社区' : (it.source === 'bundle' ? '配置包' : '手动'));
            var st = busy[it.id];
            var btns = React.createElement('button', { className: 'pcs-btn uninstall', disabled: st === 'loading', onClick: function () { onUninstall(it.id); } }, st === 'loading' ? '加载中…' : '获取卸载提示词');
            ...
            return React.createElement('div', { key: it.id, className: 'pcs-card' },
              React.createElement('div', { className: 'pcs-main' },
                React.createElement('div', { className: 'pcs-row' },
                  ...
                  React.createElement('span', { className: 'pcs-badge' }, srcLabel)),
                React.createElement('div', { className: 'pcs-meta' },
