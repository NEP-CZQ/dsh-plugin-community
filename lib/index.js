...
export function apply(ctx) {
  ...
  function parsePatchEntries(text) {
    ...
    for (const line of lines) {
      const t = line.trim()
      if (t.startsWith('- id: ')) {
        current = { id: t.slice(6).trim() }
      } else if (current && t.startsWith('name: ')) {
        const name = t.slice(6).trim().replace(/^['"]|['"]$/g, '')
        current.name = name
  ...
  async function fetchCommunity() {
    ...
    try {
      ...
      const repos = (data.items || []).map(function (r) {
        ...
        return repo
      }).filter(function (repo) { return repo.category !== '目录聚合' })
  ...
  async function readProfileBundles() {
    try {
      const pkgPath = dshHome() + '/profiles/web/package.json'
      const target = await fs.resolve(pkgPath)
      const text = await fs.readText(target)
      const pkg = JSON.parse(text)
      const bundles = (pkg && pkg.dsh && pkg.dsh.profile && pkg.dsh.profile.bundles) || []
      return bundles.filter(function (b) { return b !== '@deepseek-ai/dsh-base' && b !== '@deepseek-ai/dsh-web-app' })
    } catch (e) { return [] }
  }
  ...
  async function doList() {
    ...
    // 与真实配置对账：build actual id set（patch + bundles + 精选）
    const actual = {}
    for (const e of CATALOG) {
      if (isInstalledIn(text, e.id)) actual[e.id] = true
    }
    for (const pe of parsePatchEntries(text)) {
      if (!byId[pe.id]) actual[pe.id] = true
    }
    const bundles = await readProfileBundles()
    for (const b of bundles) {
      actual[String(b).split('/').pop()] = true
    }
    let changed = false
    // 删除真实配置中已不存在的过期记录
    for (const id of Object.keys(inst)) {
      if (!actual[id]) { delete inst[id]; changed = true }
    }
    // 补齐缺失记录
    for (const e of CATALOG) {
      if (actual[e.id] && !inst[e.id]) {
        inst[e.id] = { packageName: e.row.name, source: 'official', installedVersion: await installedVersionOf(e.row.name) }
        changed = true
      }
    }
    for (const pe of parsePatchEntries(text)) {
      if (actual[pe.id] && !inst[pe.id]) {
        inst[pe.id] = { packageName: pe.name || pe.id, source: 'manual', installedVersion: await installedVersionOf(pe.name) }
        changed = true
      }
    }
    for (const b of bundles) {
      const bid = String(b).split('/').pop()
      if (actual[bid] && !inst[bid]) {
        inst[bid] = { packageName: b, source: 'bundle', installedVersion: await installedVersionOf(b) }
        changed = true
      }
    }
    if (changed) await saveInstalled(inst)
  ...
  async function doInstall(id) {
    ...
    const prompt = '在 $DSH_HOME/profiles/web/cordis.patch.yml 追加：' + NL +
      '- insert:' + NL +
      '    - id: ' + entry.id + NL +
      '      name: ' + JSON.stringify(entry.row.name) + NL +
      '然后重启 dsh web 并刷新页面。'
  ...
  async function doUninstall(id) {
    const text = await readPatchText()
    const inPatch = parsePatchEntries(text).some(function (p) { return p.id === id })
    const bundles = await readProfileBundles()
    const bundleMatch = bundles.find(function (b) { return String(b).split('/').pop() === id })
    if (!inPatch && !bundleMatch) return { ok: false, error: 'unknown plugin: ' + String(id) }
    if (bundleMatch) {
      const prompt = '在 $DSH_HOME/profiles/web/package.json 中：' + NL +
        '1) 从 dsh.profile.bundles 移除 ' + bundleMatch + NL +
        '2) 从 dependencies 移除对应条目' + NL +
        '3) 重启 dsh web 并刷新页面。'
      return { ok: true, prompt: prompt }
    }
    const prompt = '在 $DSH_HOME/profiles/web/cordis.patch.yml 删除 id 为 ' + id + ' 的 - insert 条目，' + NL + '然后重启 dsh web 并刷新页面。'
    return { ok: true, prompt: prompt }
  }
  ...
  async function doCommunityInstall(args) {
    ...
    if (typeof fetch === 'function') {
      try {
        ...
        if (res.ok) {
          const pkg = await res.json()
          if (pkg && typeof pkg.name === 'string') pkgName = pkg.name
        }
      } catch (e) {}
    }

    if (pkgName) {
      const prompt = '1) 安装插件：dsh plugin --profile web add ' + pkgName + NL +
        '   （若仓库 README「快速开始」里的包名不同，以 README 为准）' + NL +
        '2) 重启 dsh web 并刷新页面。' + NL +
        '仓库网址：' + repoUrl
      return { ok: true, prompt: prompt }
    }

    const prompt = '该插件不是 npm 包，采用「仓库直装」：' + NL +
      '1) git clone ' + repoUrl + NL +
      '2) 按仓库 README 的安装说明操作（常见：dsh plugin add <克隆路径>，或把插件条目写入 cordis.patch.yml）' + NL +
      '3) 重启 dsh web 并刷新页面。' + NL +
      '仓库网址：' + repoUrl
    return { ok: true, prompt: prompt }
  }
  ...
  async function doUpdate(id) {
    const inst = await readInstalled()
    const rec = inst[id]
    if (!rec) return { ok: false, error: 'unknown installed plugin: ' + String(id) }
    const pkg = rec.packageName
    const isGlobal = rec.source === 'official'
    const cmd = (isGlobal ? 'npm install -g ' : 'npm install ') + pkg + '@latest'
    const prompt = '更新命令：' + NL + cmd + NL + '（官方插件版本需与 harness 兼容，谨慎升级；更新后重启 dsh web）'
    return { ok: true, prompt: prompt }
  }

  async function doRaw(args) {
    const url = args && args.url
    if (!url) return { ok: false, error: 'missing url' }
    if (!/^https:\/\/(raw\.githubusercontent\.com|api\.github\.com)\//.test(url)) {
      return { ok: false, error: 'only raw.githubusercontent.com / api.github.com URLs allowed' }
    }
  ...
  function route(handler) {
    return async function (req, res) {
      ...
      try { await handler(req, res) } catch (e) { send(res, 500, { error: String(e && e.message || e) }) }
    }
  }
  ...
  ctx.effect(function () {
    const routes = [
      webServer.register({ kind: 'exact', path: '/plugin-community/list', handler: route(async function (req, res) { send(res, 200, await doList()) }) }),
      webServer.register({ kind: 'exact', path: '/plugin-community/install', handler: route(async function (req, res) { const b = await bodyOf(req); send(res, 200, await doInstall(b && b.id)) }) }),
      webServer.register({ kind: 'exact', path: '/plugin-community/uninstall', handler: route(async function (req, res) { const b = await bodyOf(req); send(res, 200, await doUninstall(b && b.id)) }) }),
      webServer.register({ kind: 'exact', path: '/plugin-community/install-community', handler: route(async function (req, res) { const b = await bodyOf(req); send(res, 200, await doCommunityInstall(b)) }) }),
      webServer.register({ kind: 'exact', path: '/plugin-community/update', handler: route(async function (req, res) { const b = await bodyOf(req); send(res, 200, await doUpdate(b && b.id)) }) }),
      webServer.register({ kind: 'exact', path: '/plugin-community/raw', handler: route(async function (req, res) { const b = await bodyOf(req); send(res, 200, await doRaw(b)) }) })
    ]
