...
export function apply(ctx) {
  ...
  async function translateBatch(texts) {
    ...
    let model
    if (adm) {
      try { const sel = adm.currentSelection(); provider = sel && sel.provider; model = sel && sel.model } catch (e) {}
    }
    if (!provider || !model) return out
    for (const t of texts) { if (t && TRANSLATION_CACHE.has(t)) out[t] = TRANSLATION_CACHE.get(t) }
    const pending = texts.filter(function (t) { return t && !TRANSLATION_CACHE.has(t) })
    if (pending.length === 0) return out
    try {
      const prompt = '把下面每行英文插件简介翻译成简体中文。每行对应输出一行译文，保持顺序，不要编号、不要解释：' + NL + pending.join(NL)
      const messages = [createUserMessage({ content: [{ type: 'text', text: prompt }], source: { kind: 'plugin', plugin: 'dsh-plugin-community' } })]
      const options = { provider: provider, model: model, messages: messages, maxTokens: 2000 }
      const assembler = new BlockAssembler()
      for await (const chunk of llm.stream(options)) { assembler.push(chunk) }
      if (!assembler.finish || assembler.finish.kind !== 'stop') return out
      const text = assembler.blocks().filter(function (b) { return b.type === 'text' }).map(function (b) { return b.text }).join('')
      const lines = text.split(NL).map(function (s) { return s.trim() }).filter(function (s) { return s.length > 0 })
      for (let i = 0; i < pending.length && i < lines.length; i++) {
        if (lines[i]) { TRANSLATION_CACHE.set(pending[i], lines[i]); out[pending[i]] = lines[i] }
      }
    } catch (e) {}
    return out
  }
  ...
  async function fetchCommunity() {
    ...
    try {
      ...
      for (const r of repos) {
        ...
        if (d && /[a-zA-Z]/.test(d) && !seen[d]) { seen[d] = true; descs.push(d) }
      }
      const zh = await translateBatch(descs)
      for (const r of repos) {
        r.descZh = zh[r.description] || ''
      }
  ...
  async function doUninstall(id) {
    const entry = byId[id]
    if (!entry) return { ok: false, error: 'unknown plugin: ' + String(id) }
    const prompt = '在 $DSH_HOME/profiles/web/cordis.patch.yml 删除 id 为 ' + entry.id + ' 的 - insert 条目，' + NL + '然后重启 dsh web 并刷新页面。'
    return { ok: true, prompt: prompt }
  }
