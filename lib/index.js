...
export function apply(ctx) {
  ...
  async function doUpdate(id) {
    ...
    try {
      const t = await fs.resolve(stagedPath)
      await fs.writeText(t, script)
      staged = true
    } catch (e) { stageError = String(e && e.message ? e.message : e) }
    return { ok: true, command: cmd, staged: staged, stagedPath: stagedPath, stageError: stageError, needsAuthorization: true, packageName: pkg, source: rec.source }
  }
