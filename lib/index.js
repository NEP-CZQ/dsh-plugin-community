...
export function apply(ctx) {
  ...
  async function doCommunityInstall(args) {
    ...
    if (pkgName) {
      const rowId = String(pkgName).split('/').pop()
      const prompt = '1) 先安装包：npm install ' + pkgName + '@latest' + NL +
        '2) 在 $DSH_HOME/profiles/web/cordis.patch.yml 追加：' + NL +
        '- insert:' + NL +
        '    - id: ' + rowId + NL +
        '      name: ' + JSON.stringify(pkgName) + NL +
        '3) 重启 dsh web 并刷新页面。'
      return { ok: true, prompt: prompt }
    }
