...
export function apply(ctx) {
  ...
  async function doCommunityInstall(args) {
    ...
    try {
      ...
      const res = await fetch(url, { headers: { 'user-agent': 'dsh-plugin-community-store' } })
      if (!res.ok) return { ok: false, error: '该仓库没有根 package.json（HTTP ' + res.status + '），不是可安装的 npm 包；请确认它是 DSH 插件并按其 README 安装' }
      pkg = await res.json()
    } catch (e) {
      return { ok: false, error: String(e && e.message ? e.message : e) }
    }
    const name = pkg && typeof pkg.name === 'string' ? pkg.name : ''
    if (!name) return { ok: false, error: '该仓库 package.json 未声明 name（可能不是可安装插件），请按其 README 手动安装' }
    const rowId = String(name).split('/').pop()
    const prompt = '1) 先安装包：npm install ' + name + '@latest' + NL +
      '2) 在 $DSH_HOME/profiles/web/cordis.patch.yml 追加：' + NL +
      '- insert:' + NL +
      '    - id: ' + rowId + NL +
      '      name: ' + JSON.stringify(name) + NL +
      '3) 重启 dsh web 并刷新页面。'
    return { ok: true, prompt: prompt }
  }
