...
export function apply(ctx) {
  ...
  async function doUpdate(id) {
    ...
    const script = '@echo off' + NL + cmd + NL
    const stagedPath = WORKSPACE + '/.plugin-community-update-' + id + '.cmd'
    const stage = async function () {
      try { const t = await fs.resolve(stagedPath); await fs.writeText(t, script); return true } catch (e) { return false }
    }
    const shell = ctx.get('shell')
    if (shell) {
      try {
        const spec = shell.resolve({ command: cmd, workdir: dshHome(), timeoutMs: 300000 })
        const result = await shell.run(spec)
        const exitCode = result.exitCode
        const out = (result.stdout && result.stdout.text) || ''
        const err = (result.stderr && result.stderr.text) || ''
        if (exitCode === 0) {
          return { ok: true, executed: true, command: cmd, exitCode: 0, stdout: out }
        }
        const staged = await stage()
        return { ok: false, executed: false, command: cmd, exitCode: exitCode, stderr: err, staged: staged, stagedPath: stagedPath, needsAuthorization: true }
      } catch (e) {
        const staged = await stage()
        return { ok: false, executed: false, command: cmd, error: String(e && e.message ? e.message : e), staged: staged, stagedPath: stagedPath, needsAuthorization: true }
      }
    }
    const staged = await stage()
    return { ok: true, executed: false, command: cmd, staged: staged, stagedPath: stagedPath, needsAuthorization: true, packageName: pkg, source: rec.source }
  }
  ...
  ctx.effect(function () {
    const routes = [
      ...
      webServer.register({ kind: 'exact', path: '/plugin-community/update', handler: async function (req, res) { const b = await bodyOf(req); try { send(res, 200, await doUpdate(b && b.id)) } catch (e) { send(res, 500, { error: String(e && e.message || e) }) } } })
    ]
